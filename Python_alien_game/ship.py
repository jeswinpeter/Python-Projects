...
class Ship:
    ...
    def __init__(self, ai_game):
        ...
        # Movement flag
        self.movivig_right = False
        self.movivig_left = False

    def update(self):
        """Update the ship's position based on the movement flag."""
        # Update the ships x value
        if self.movivig_right and self.rect.right < self.screen_rect.right:
            self.x += self.settings.ship_speed
        if self.movivig_left and self.rect.left > 0:
            self.x -= self.settings.ship_speed
